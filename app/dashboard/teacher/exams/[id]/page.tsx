...
export default function TeacherExamDetailPage() {
  ...
  const classPublished = useMemo(
    () => sheets.length > 0 && sheets.every((s) => s.isPublished),
    [sheets],
  );
  ...
  async function save() {
    ...
    for (const s of sheets) {
      ...
      const maxMarks = num(s.maxMarks);
      const original = s.marksObtained == null ? null : num(s.marksObtained);
  ...
  async function publishClass(publish: boolean) {
    ...
    setMessage("");
    try {
      const res = await examsApi.publish(examId, { publish, classId });
      setMessage(
        publish
  ...
  async function onSaveEdit(e: React.FormEvent) {
    ...
    const classIds = [...new Set(papersPayload.map((p) => p.classId))];
  ...
  async function onDelete() {
    ...
    try {
      ...
      router.replace("/dashboard/teacher/exams");
    } catch (err) {
    ...
    return (
      <DashboardShell allowedRoles={["TEACHER"]}>
        ...
        <Button asChild variant="outline" className="mt-4">
          <Link href="/dashboard/teacher/exams">Back</Link>
        </Button>
      </DashboardShell>
    );
  }
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <Badge variant={classPublished ? "default" : "secondary"}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Spreadsheet view — subjects as columns. Publish applies to the
              whole class (your subjects).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {studentRows.length === 0 ? (
              <p className="text-muted-foreground text-sm">
                No mark sheets for this class (or you do not have access).
              </p>
            ) : (
              <MarkSheetGrid
            ...
            <form onSubmit={onSaveEdit}>
              ...
              <div className="mt-4 space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    <Label htmlFor="td-edit-name">Name</Label>
                    <Input
                      id="td-edit-name"
                      value={editForm.name}
                      onChange={(e) =>
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="td-edit-date">Exam date</Label>
                    <Input
                      id="td-edit-date"
                      type="date"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="td-edit-desc">Description</Label>
                  <Textarea
                    id="td-edit-desc"
